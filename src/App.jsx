...
export default function App() {
  ...
  const [browsingCategory, setBrowsingCategory] = useState(null); // null = show category cards, or category key to show playbooks
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-50">
      ...
      <main className="max-w-5xl mx-auto px-4 py-10">
        ...
        <div className="flex flex-wrap gap-2 mb-8 p-1 bg-slate-100 rounded-2xl w-fit">
          <button
            onClick={() => { setSection('playbooks'); setSearch(''); setBrowsingCategory(null); }}
          ...
          <button
            onClick={() => { setSection('templates'); setSearch(''); setBrowsingCategory(null); }}
            className={`flex items-center gap-2 px-5 py-3 rounded-xl text-sm font-bold transition-all ${
          ...
          <button
            onClick={() => { setSection('cheatsheets'); setSearch(''); setBrowsingCategory(null); }}
            className={`flex items-center gap-2 px-5 py-3 rounded-xl text-sm font-bold transition-all ${
        ...
        {section === 'playbooks' && (<>
          {/* Category drill-down view (default) */}
          {!browsingCategory && !search && (
            <>
              <div className="mb-8 text-center">
                <h2 className="text-2xl md:text-3xl font-bold text-slate-900 mb-3">Pick your discipline</h2>
                <p className="text-slate-600 max-w-2xl mx-auto">Three categories. {PLAYBOOKS.length} playbooks. Choose where you want to focus, or search across everything.</p>
              </div>
              ...
              <div className="relative mb-8 max-w-2xl mx-auto">
                ...
                <input
                  ...
                  placeholder="Or search all playbooks..."
              ...
              <div className="grid gap-6 md:grid-cols-3 mb-12">
                {Object.entries(CATEGORIES).map(([key, cat]) => {
                  ...
                  const Icon = cat.icon;
                  const totalMinutes = PLAYBOOKS.filter(p => p.category === key).reduce((sum, p) => sum + p.estMinutes, 0);
                  const totalSteps = PLAYBOOKS.filter(p => p.category === key).reduce((sum, p) => sum + p.steps.length, 0);

                  return (
                    <button
                      key={key}
                      onClick={() => setBrowsingCategory(key)}
                      className="relative group overflow-hidden rounded-3xl bg-white border-2 border-slate-200 hover:border-slate-300 p-8 text-left transition-all hover:shadow-2xl hover:-translate-y-2 min-h-[320px] flex flex-col"
                    >
                      <div className={`absolute top-0 left-0 right-0 h-2 bg-gradient-to-r ${cat.gradient}`} />
                      <Illustration className="absolute -right-6 -top-6 w-44 h-44 opacity-40 group-hover:opacity-70 group-hover:scale-110 transition-all duration-500" />

                      <div className="relative flex-1">
                        <div className={`inline-flex items-center justify-center w-14 h-14 rounded-2xl bg-gradient-to-br ${cat.gradient} text-white shadow-lg mb-4`}>
                          <Icon className="w-7 h-7" />
                        </div>

                        <h3 className="text-2xl font-bold text-slate-900 mb-2 leading-tight">{cat.label}</h3>
                        <p className="text-slate-600 leading-relaxed text-sm mb-4">
                          {key === 'change' && 'Move people through change with empathy, structure, and the right toolkit.'}
                          {key === 'ba' && 'Surface real requirements, map processes, and build cases that win funding.'}
                          {key === 'pi' && 'Find the root cause, target the waste, and make improvements stick.'}
                        </p>
                      </div>

                      <div className="relative flex items-center justify-between pt-4 border-t-2 border-slate-100">
                        <div className="flex items-center gap-3 text-sm">
                          <span className="font-bold text-slate-900">{count} playbooks</span>
                          <span className="w-1 h-1 bg-slate-300 rounded-full" />
                          <span className="text-slate-500">{totalSteps} steps</span>
                          <span className="w-1 h-1 bg-slate-300 rounded-full" />
                          <span className="text-slate-500">~{Math.round(totalMinutes/60)}h total</span>
                        </div>
          ...
          {/* Inside-a-category view OR search results */}
          {(browsingCategory || search) && (
            <>
              ...
              {browsingCategory && !search && (
                ...
                >
                  ...
                  <span className="text-sm font-medium">All disciplines</span>
                </button>
              )}

              {/* Category header when drilled in */}
              {browsingCategory && !search && (() => {
                const cat = CATEGORIES[browsingCategory];
                const Icon = cat.icon;
                const count = PLAYBOOKS.filter(p => p.category === browsingCategory).length;
                return (
                  <div className={`relative overflow-hidden rounded-3xl bg-gradient-to-br ${cat.gradient} p-6 md:p-8 text-white mb-8 shadow-xl`}>
                    <div className="absolute -top-12 -right-12 w-44 h-44 rounded-full bg-white/10 blur-3xl pointer-events-none" />
                    <div className="relative">
                      <div className="inline-flex items-center justify-center w-14 h-14 rounded-2xl bg-white/20 backdrop-blur-sm mb-3">
                        <Icon className="w-7 h-7" />
                      </div>
                      <h2 className="text-3xl md:text-4xl font-bold mb-2 leading-tight">{cat.label}</h2>
                      <p className="text-white/90 leading-relaxed">
                        {browsingCategory === 'change' && `${count} playbooks for leading people through change — from stakeholder analysis through to post-implementation review.`}
                        {browsingCategory === 'ba' && `${count} playbooks for business analysts — requirements, process mapping, design, and decision-making.`}
                        {browsingCategory === 'pi' && `${count} playbooks for process improvement practitioners — root cause analysis, value stream mapping, and continuous improvement.`}
                      </p>
                    </div>
                  </div>
                );
              ...
              {/* Search input when in category or searching */}
              <div className="mb-8 space-y-4">
                <div className="relative">
                  ...
                  <input
                    ...
                    value={search}
                    onChange={(e) => setSearch(e.target.value)}
                    placeholder={browsingCategory ? `Search ${CATEGORIES[browsingCategory].label.toLowerCase()}...` : "Search playbooks..."}
                    className="w-full pl-12 pr-4 py-4 rounded-2xl border-2 border-slate-200 bg-white focus:outline-none focus:border-violet-500 focus:ring-4 focus:ring-violet-100 transition text-slate-900 placeholder:text-slate-400 font-medium"
                  />
                </div>
              </div>
              ...
              <div className="grid gap-5 md:grid-cols-2">
                {PLAYBOOKS.filter(p => {
                  const matchesSearch = !search || p.title.toLowerCase().includes(search.toLowerCase()) || p.description.toLowerCase().includes(search.toLowerCase());
                  const matchesCategory = !browsingCategory || p.category === browsingCategory;
              ...
              {PLAYBOOKS.filter(p => {
                ...
                <div className="text-center py-20 bg-white rounded-3xl border-2 border-dashed border-slate-200">
                  ...
                  <p className="text-slate-500 text-sm">Try a different keyword or browse another discipline</p>
                </div>
              )}
            </>
          )}
